...
struct BMP * getBMP(FILE *fp)
{
  ...
  {
    bmp->color_table = (uint32_t *)malloc(sizeof(uint32_t) * bmp->colors_in_table);
  ...
  bmp->data = (uint8_t *)malloc(sizeof(uint8_t) * bmp->height * bmp->width);

  fread((void *)bmp->data, sizeof(uint8_t) * bmp->height * bmp->width, 1, fp);
...
void print_BMP(struct BMP * bmp)
{
  ...
  i=0;
  while(i < (bmp->height * bmp->width * (bmp->bits_per_pixel >> 3)))
  {
